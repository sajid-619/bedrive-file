...
import React, { createContext, useEffect, useMemo, useCallback, cloneElement, useContext, useState, useRef, Fragment, memo, forwardRef, Children, isValidElement, useId, useLayoutEffect as useLayoutEffect$1 } from "react";
import { aT as useMediaQuery, ay as getFromLocalStorage, az as setInLocalStorage, aY as Underlay, e as createSvgIcon, aC as Navbar, d as IconButton, aA as getBootstrapData, m as message, M as getInputFieldClassNames, ag as useField, ah as Field, aZ as useUserTimezone, b as useIsMobileMediaQuery, a_ as useSelectedLocale, a$ as useDateFormatter, u as useSettings, ad as DateFormatPresets, aR as shallowEqual, W as List, X as ListItem, T as Trans, f as useDialogContext, y as DialogFooter, B as Button, D as Dialog, h as DialogBody, j as DialogTrigger, b0 as AvatarPlaceholderIcon, L as Tooltip, O as createEventHandler, b1 as useIsDarkMode, p as Skeleton, i as Checkbox, k as useTrans, Z as FormSelect, I as Item, a as apiClient, S as SelectForwardRef, o as opacityAnimation, A as FormTextField, H as Chip, b2 as useListbox, b3 as Listbox, b4 as Popover, a2 as ProgressCircle, K as KeyboardArrowDownIcon, b5 as useListboxKeyboardNavigation, g as DialogHeader, F as Form, c as useNumberFormatter, G as CloseIcon, b6 as useAutoFocus, aH as UploadedFile, b7 as isAbsoluteUrl, am as FileTypeIcon } from "../server-entry.mjs";
...
import { E as EditIcon, C as ChevronRightIcon } from "./Edit-ce7ae1ac.mjs";
...
function useTableRowStyle({ index, isSelected, isHeader }) {
  ...
  const isMobile = useIsMobileMediaQuery();
  const { hideBorder, enableSelection, collapseOnMobile } = useContext(TableContext);
  const isFirst = index === 0;
  return clsx(
    ...
    !isSelected && !isHeader && enableSelection && "focus-visible:bg-focus hover:bg-hover"
  );
}
...
function ChipList({
  ...
  return /* @__PURE__ */ jsx("div", { className: clsx(className, "flex items-center gap-8 flex-wrap"), children: Children.map(children, (chip) => {
    ...
    if (item.kind === "file") {
      if (typeof item.webkitGetAsEntry === "function") {
        ...
        if (!file || file.name === ".DS_Store")
          continue;
        yield new UploadedFile(file, item.fullPath);
      }
    }
  }
}
...
//# sourceMappingURL=UnfoldMore-a9683c4c.mjs.map
